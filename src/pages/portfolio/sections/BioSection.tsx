...
function StatBar({ label, pct, theme }: { label: string; pct: number; theme: PortfolioTheme }) {
  ...
  /* ── Comic chunky bar ── */
  if (isComic) {
    const barColor = pct >= 85 ? theme.primary : pct >= 75 ? theme.accent : theme.secondary;
    return (
      <div style={{ marginBottom: '0.85rem' }}>
        <div style={{
          display: 'flex', justifyContent: 'space-between',
          fontFamily: theme.fontBody, fontWeight: 700, fontSize: '0.9rem',
          color: theme.cardText, marginBottom: '4px',
        }}>
  ...
  /* ── Newspaper thin editorial bar ── */
  if (isNewspaper) {
    return (
      <div style={{ marginBottom: '0.75rem' }}>
        <div style={{
          display: 'flex', justifyContent: 'space-between',
          fontFamily: theme.fontBody, fontStyle: 'italic', fontSize: '0.82rem',
          color: theme.cardDim, marginBottom: '4px',
        }}>
          <span>{label}</span>
          <span style={{ fontStyle: 'normal', fontWeight: 700, color: theme.primary }}>{pct}%</span>
        </div>
        <div style={{
  ...
  if (isCrossing) {
    ...
    return (
      <div style={{ marginBottom: '0.75rem' }}>
        <div style={{
          display: 'flex', justifyContent: 'space-between',
          fontFamily: theme.fontBody, fontWeight: 600, fontSize: '0.9rem',
          color: theme.cardDim, marginBottom: '3px',
        }}>
          <span>{label}</span>
        </div>
  ...
  return (
    <div style={{ marginBottom: '0.6rem' }}>
      <div style={{ fontFamily: theme.fontBody, fontSize: '0.78rem', color: theme.cardDim, marginBottom: '2px', letterSpacing: '0.05em' }}>
        {label.toUpperCase()}
      </div>
      <div style={{ fontFamily: theme.fontBody, fontSize: '0.85rem', color: theme.secondary, letterSpacing: '2px' }}>
...
export function BioSection() {
  ...
  else if (isCrossing)  sectionBg = 'linear-gradient(180deg, #d4ecc4 0%, #dff0d0 100%)';
  ...
  return (
    ...
    >
      <SectionHeading label={theme.sectionLabel('01', 'About Me')} theme={theme} />

      <div style={{
        display: 'flex', gap: '2.5rem', maxWidth: '900px', width: '100%',
        ...
        <motion.div
          ...
          style={{ flex: '0 0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.75rem' }}
        >
          {/* ── Comic avatar: thick-bordered character portrait ── */}
          {isComic ? (
            <>
              <div style={{
            ...
            /* ── Newspaper avatar: simple bordered square, greyscale ── */
            <>
              <div style={{
                width: '180px', height: '180px',
                background: '#E8E4D8',
                ...
                <div style={{
                  position: 'absolute', inset: 0,
                  backgroundImage: `repeating-linear-gradient(0deg, #AAAAAA22 0px, transparent 1px, transparent 20px),
                                    repeating-linear-gradient(90deg, #AAAAAA22 0px, transparent 1px, transparent 20px)`,
                }} />
                <div style={{
                  position: 'absolute', top: '50%', left: '50%',
                  transform: 'translate(-50%,-50%)',
            ...
            <>
              <div style={{
                width: '180px', height: '180px',
                background: isCrossing ? theme.surface : theme.surface,
                position: 'relative', overflow: 'hidden',
                ...(isCrossing
                  ? {
                      borderRadius: '50%',
                ...
                {isCrossing ? (
                  ...
                  <>
                    ...
                    <div style={{
                      position: 'absolute', top: '50%', left: '50%',
                      width: '8px', height: '8px', background: theme.primary, borderRadius: '50%',
                      transform: 'translate(-50%,-50%)', boxShadow: `0 0 12px ${theme.primary}`,
                    }} />
                  </>
                )}
              </div>
        ...
        >
          ...
          <div style={{ padding: '1.2rem 1.4rem 0.5rem' }}>
            {DOSSIER.map(({ label, value, icon }) => (
              <div key={label} style={{
                ...
                fontFamily: theme.fontBody,
                fontSize: isComic ? '0.95rem' : isNewspaper ? '0.88rem' : isCrossing ? '0.95rem' : '0.82rem',
                ...
                <span style={{ color: theme.cardText, fontWeight: isComic ? 700 : 400 }}>{value}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ padding: '0.25rem 1.4rem 1.4rem' }}>
            <div style={{
              fontFamily: isNewspaper ? theme.fontHeading : theme.fontHeading,
              fontWeight: 800,
              fontStyle: isNewspaper ? 'italic' : 'normal',
              fontSize: isComic ? '1rem' : isNewspaper ? '0.95rem' : isCrossing ? '0.9rem' : '0.75rem',
              color: isComic ? theme.primary : isNewspaper ? theme.secondary : isCrossing ? theme.secondary : theme.primary,
              marginBottom: '0.75rem',
              letterSpacing: isComic ? '0.04em' : isCrossing ? '0.02em' : '0.15em',
              display: 'flex', alignItems: 'center', gap: '0.4rem',
              borderBottom: isNewspaper ? '1px solid #AAAAAA' : 'none',
              paddingBottom: isNewspaper ? '0.4rem' : '0',
            }}>
              {isComic
                ? 'POWER STATS'
                : isNewspaper
